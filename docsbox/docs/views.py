    ...
    def post(self, file_id):
        ...
        try:
            if request.files and "file" in request.files:
                ...
                file_info = get_file_info(file_id, request.headers.get('Content-Disposition'), request.headers.get('Content-Type'), save_file=True)
    ...
    def post(self, file_id):
        ...
        try:
            if request.files and "file" in request.files:
                ...
                file_path = store_file(request.files["file"], filename)
                mimetype = get_file_mimetype(file_path)
                task = process_convertion.queue(file_path, set_options(request.headers, mimetype), {"filename": remove_extension(filename), "mimetype": mimetype, "save_in_via": False})
...
def get_file_info(file_id, filename="", mimetype=None, save_file=False):
    file_info = {}
    if db.exists('fileId:' + file_id) == 0:
        file_info = {"file_id": file_id,"mimetype": mimetype, "filename": filename, "datetime": datetime.now().strftime('%Y/%m/%d-%H:%M:%S')}
